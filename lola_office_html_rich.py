"""High-fidelity Office -> interactive HTML renderer."""
from __future__ import annotations
import base64, html
from pathlib import Path
from lola_office_html_styles import xlsx_style, xlsx_images, docx_headers_footers
...
def _chart_inventory(ws):
    return "".join("<div class='chart-card' data-chart='%d'>Chart %d · workbook chart metadata detected</div>"%(i,i)
                   for i,_ in enumerate(getattr(ws,"_charts",[]),1))
    ...
    for ws in wb.worksheets:
        ...
        for r in range(1,ws.max_row+1):
            ...
            for c in range(1,ws.max_column+1):
                ...
                cells.append("<td"+attrs+" data-cell='"+cell.coordinate+"' style='"+
                             html.escape(xlsx_style(cell),quote=True)+"'>"+html.escape(value)+"</td>")
...
def save_rich(src,dst,kind):
    ...
    p.write_text(interactive_shell(Path(src).name,body),encoding="utf-8")
    report=verify(src,p)
    diagnostics=diagnose(src,p)
    report_path=p.with_suffix(".verification.json")
    save_report({"verification":report,"diagnostics":diagnostics},report_path)
    return {"ok":True,"output":str(p),"mode":"interactive-rich-html",
            "verification":report,"diagnostics":diagnostics,"verification_file":str(report_path)}
